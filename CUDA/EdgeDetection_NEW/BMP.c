...
void BMP_Init(BMP *bmp, const char *input_image_path)
{
    ...
    BMP_ExtractImageData(input_image_path, bmp);
    printf("%s loaded: %dx%d\n", input_image_path, bmp->width, bmp->height);
    // Convert the input bmp 1D image data to 2D image data for the pooling processing.
...
void BMP_SaveGrayscaleImage(const char *filename, uint8_t *image, uint16_t width, uint16_t height)
{
    ...
    _BMP_CreateHeaders(bmp_header, bmp_dib_header, bmp_color_palette, width, height);
    ...
    {
        fwrite(&image[y * width], 1, width, file);        
        fwrite(padding_bytes, 1, padding, file);
    }

    fclose(file);

    printf("Output image saved at: %s\r\n", filename);
}
