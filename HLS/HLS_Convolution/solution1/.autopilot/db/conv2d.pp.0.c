# 1 "HLS_Convolution/sources/conv2d.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 360 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "HLS_Convolution/sources/conv2d.c" 2
...
# 90 "C:\\Xilinx\\Vitis_HLS\\2024.1\\win64\\tools\\clang-3.9-csynth\\lib\\clang\\7.0.0\\include\\stddef.h" 3
typedef unsigned short wchar_t;
...
# 2 "HLS_Convolution/sources/conv2d.c" 2
...
#pragma pack(push,_CRT_PACKING)
# 119 "C:/Xilinx/Vitis_HLS/2024.1/tps/mingw/8.3.0/win64.o/nt\\x86_64-w64-mingw32\\include\\math.h" 3
...
# 1581 "C:/Xilinx/Vitis_HLS/2024.1/tps/mingw/8.3.0/win64.o/nt\\x86_64-w64-mingw32\\include\\math.h" 3
#pragma pack(pop)
# 3 "HLS_Convolution/sources/conv2d.c" 2
# 14 "HLS_Convolution/sources/conv2d.c"
__attribute__((sdx_kernel("conv2d", 0))) void conv2d(uint8_t *in_image, uint8_t out_image[((128 - 3) + 1)][((128 - 3) + 1)])
{
#line 16 "C:/DevWorks_HLS/Convolution_Project/HLS_Convolution/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=conv2d
# 15 "HLS_Convolution/sources/conv2d.c"

#line 7 "C:/DevWorks_HLS/Convolution_Project/HLS_Convolution/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=conv2d
# 15 "HLS_Convolution/sources/conv2d.c"
...
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
 ...
 int16_t out_image_x[((128 - 3) + 1)][((128 - 3) + 1)];
    int16_t out_image_y[((128 - 3) + 1)][((128 - 3) + 1)];

    uint16_t out_image_sobel[((128 - 3) + 1)][((128 - 3) + 1)];

    const int16_t sample_y[3][3] = {
        {1, 0, -1},
        {2, 0, -2},
        {1, 0, -1}
    };
    const int16_t sample_x[3][3] = {
        {1, 2, 1},
        {0, 0, 0},
        {-1, -2, -1}
    };
    ...
    VITIS_LOOP_39_1: for (int row = 0; row < (128 - 3 + 1); ++row) {
        VITIS_LOOP_40_2: for (int col = 0; col < (128 - 3 + 1); ++col) {
            int16_t sum = 0;

            VITIS_LOOP_43_3: for (int i = 0; i < 3; ++i) {
                VITIS_LOOP_44_4: for (int j = 0; j < 3; ++j) {
                    int img_row = row + i;
                    int img_col = col + j;
                    sum += sample_y[i][j] * in_image[img_row * 128 + img_col];
                }
            }
    ...
    VITIS_LOOP_56_5: for (int row = 0; row < (128 - 3 + 1); ++row) {
        VITIS_LOOP_57_6: for (int col = 0; col < (128 - 3 + 1); ++col) {
            int16_t sum = 0;

            VITIS_LOOP_60_7: for (int i = 0; i < 3; ++i) {
                VITIS_LOOP_61_8: for (int j = 0; j < 3; ++j) {
                    int img_row = row + i;
                    int img_col = col + j;
                    sum += sample_x[i][j] * in_image[img_row * 128 + img_col];
                }
            }
    ...
    VITIS_LOOP_73_9: for (int i = 0; i < ((128 - 3) + 1); ++i) {
        VITIS_LOOP_74_10: for (int j = 0; j < ((128 - 3) + 1); ++j) {
 ...
 uint16_t max_val = 0;
    ...
    VITIS_LOOP_85_11: for (uint16_t row = 0; row < 128 ; ++row)
    {
        VITIS_LOOP_87_12: for (uint16_t col = 0; col < 128 ; ++col)
        {
    ...
    VITIS_LOOP_98_13: for (uint16_t row = 0; row < ((128 - 3) + 1); ++row)
    {
        VITIS_LOOP_100_14: for (uint16_t col = 0; col < ((128 - 3) + 1); ++col)
        {
